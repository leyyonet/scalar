...
export function booleanTypeSample(describe: FuncLike, it: FuncLike): void {
    const items: Array<ScalarTestingItem> = [];
    // region basic
    items.push({
        info: 'undefined value should be converted to null',
        type: 'basic',
        input: undefined,
        expected: null,
    });
    items.push({
        info: 'positive numbers should be converted to true',
        type: 'basic',
        input: 5,
        expected: true,
    });
    items.push({
        info: 'negative numbers should be converted to false',
        type: 'basic',
        input: -5,
        expected: false,
    });
    items.push({
        info: 'zero should be converted to false',
        type: 'basic',
        input: 0,
        expected: false,
    });
